...
class AdvancedMagAttention(nn.Module):
    """
    Advanced Magnitude-Aware Residual Attention.
    Uses MagFace L2-norm principles with a Deep Residual Refiner.
    """
    def __init__(self, in_features, l_a=10.0, u_a=110.0, **kwargs):
        ...
        # Branch 1: Semantic Refiner (Complex MLP with Residual and BN)
        hidden = 256
        ...
        # Residual branch to prevent gradient vanishing
        self.shortcut = nn.Linear(in_features, hidden)
    ...
    def forward(self, x):
        # 1. Physical Magnitude (The MagFace Principle)
        norm = torch.norm(x, p=2, dim=1, keepdim=True)
        
        # Normalize norm to [0, 1] range based on MagFace bounds
        mag_quality = (torch.clamp(norm, self.l_a, self.u_a) - self.l_a) / (self.u_a - self.l_a)
        
        # 2. Semantic Refinement (The Complex Architecture)
        feat = self.trunk(x) + self.shortcut(x)
        semantic_quality = self.head(feat)
        
        # 3. Best-of-Both Fusion: Range [0.6, 1.4]
        combined_score = 0.6 + (0.8 * (0.5 * mag_quality + 0.5 * semantic_quality))
        
        return combined_score, norm


ATTENTION_REGISTRY = {
    "dual_attention": AdvancedMagAttention, # CHANGED: Now points to the powerful version
}
...
class AttenDualPartialFC(nn.Module):
    """Attention-based Dual Margin Loss with MagFace Regularization"""
    def __init__(self, embedding_size, num_classes, scale=64.0, m1=0.5, m2=0.1,
                 attention_type="dual_attention", reduction=32, l_a=10.0, u_a=110.0):
    ...
    def forward(self, embeddings, labels):
        # 1. Get Quality and Norm (No detach to allow the attention to shape the backbone)
        quality_score, norm = self.attention_net(embeddings)
        ...
        # 3. MagFace Adaptive Margin
        m1_dynamic = self.m1 * quality_score 

        # 4. Target Logit calculation
        ...
        # 5. MagFace Regularization (Crucial to prevent score sticking at 0.5)
        lambda_g = 25.0 
...
# ... [REST OF YOUR SCRIPT: VALIDATION, GET_CONFIG, SETUP_LOGGING, TRAIN, MAIN] ...
...
def get_config(attention_type, dataset_path=None):
    ...
    config.batch_size = 256       # Optimized for H100
