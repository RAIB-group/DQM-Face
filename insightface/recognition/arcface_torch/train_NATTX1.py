...
class QualityMLPAttention(nn.Module):
    """
    Improved MLP Attention for Face Quality
    """
    def __init__(self, in_features, reduction=64):
        super().__init__()
        hidden1 = in_features // 2
        hidden2 = in_features // 4
    ...
    def forward(self, x):
        score = self.mlp(x)
        attention = 0.5 + 2.0 * torch.sigmoid(score)
        return attention


ATTENTION_REGISTRY = {
    "dual_attention": QualityMLPAttention,
}
...
class AttenDualPartialFC(nn.Module):
    ...
    def forward(self, embeddings, labels):
        ...
        # --- Attention ---
        atten_score = self.attention_net(embeddings.detach())
        self.last_attention_score = atten_score.mean().item()

        # --- Warm-up for attention ---
        if self.training and self.current_epoch < self.attn_warmup_epochs:
            alpha = self.current_epoch / float(self.attn_warmup_epochs)
            atten_score = 1.0 + alpha * (atten_score - 1.0)
        ...
        # Encourage non-trivial attention
        attn_reg = 0.01 * torch.mean((atten_score - 1.0) ** 2)

        return F.cross_entropy(output, labels) + attn_reg
