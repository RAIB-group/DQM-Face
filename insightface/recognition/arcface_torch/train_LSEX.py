...
class DualAttentionLSE(nn.Module):
    """Temperature-controlled LSE attention for curriculum margin scaling"""
    def __init__(self, in_features, tau_start=1.0, tau_end=0.3, total_steps=100000, **kwargs):
        super().__init__()
        self.fc1 = nn.Linear(in_features, 128)
        self.relu = nn.PReLU()
        self.fc2 = nn.Linear(128, 1)
    ...
    def forward(self, x):
        ...
        x = self.relu(x)
        x = self.fc2(x)  # [B,1]
        # Temperature-controlled LSE
        tau = self.tau_start - (self.tau_start - self.tau_end) * (self.step / self.total_steps)
        tau = max(tau.item(), self.tau_end)
        score = tau * torch.log1p(torch.exp(x / tau))  # smooth LSE
        score = torch.sigmoid(score)  # map to [0,1]
        self.step += 1
        return 0.5 + score


ATTENTION_REGISTRY = {
    "dual_attention_lse": DualAttentionLSE,
}
...
class AttenDualPartialFC(nn.Module):
    """Attention-based Dual Margin Loss with curriculum scaling"""
    def __init__(
        self,
        embedding_size,
        num_classes,
    ...
    def update_m2(self, new_m2):
        """Dynamic update for m2 repulsion margin"""
        self.m2 = new_m2

    def forward(self, embeddings, labels, global_step=None):
        ...
        # -------------------------------
        # Curriculum margin scaling
        # -------------------------------
        if global_step is not None and hasattr(self.attention_net, "total_steps"):
            lam = min(global_step / self.attention_net.total_steps, 1.0)
        else:
            lam = 1.0
        m1_dynamic = self.m1 * (1 + lam * (atten_score - 1.0))
        ...
        # -------------------------------
        # Optional: attention variance regularization
        # -------------------------------
        # atten_var_loss = -0.01 * torch.var(atten_score)
        # output_loss = F.cross_entropy(output, labels) + atten_var_loss
...
# ============================================================
# The rest of your original code remains identical:
# - Validation: load_bin, evaluate_accuracy, validate
# - Configuration: get_config, ABLATION_INFO
# - Training: setup_distributed, setup_logging, setup_seed, AverageMeter, train()
# - Main: argparse parsing, run_all / single training
# ============================================================

# (Everything else in your original script remains unchanged)
...
def get_config(attention_type, dataset_path=None):
    ...
    config.network = "r100"       # Changed from r50 to r100 (iResNet-100)
    ...
    config.lr = 0.1               # Initialized from 0.1
    config.batch_size = 512       # Optimized for H100
    config.num_epoch = 25         # Stop at 25th epoch
    config.milestones = [10, 18, 22] # Divide by 10 at these epochs
    config.warmup_epoch = 0       # Set to 0 for your specific schedule
    config.weight_decay = 5e-4    # Set to 5e-4
    config.momentum = 0.9         # Set to 0.9
...
ABLATION_INFO = {
    "dual_attention_lse": ("★ BASELINE", "Original simple MLP attention"),
}
...
def main():
    ...
    parser.add_argument(
        "--attention",
        type=str,
        default="dual_attention_lse",
        choices=["dual_attention_lse"],
    )
    ...
    if args.run_all:
        all_results = {}
        for att_type in ["dual_attention_lse"]:
            print(f"\n{'='*70}")
