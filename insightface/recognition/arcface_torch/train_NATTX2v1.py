...
class PowerMagAttention(nn.Module):
    def __init__(self, in_features, l_a=10.0, u_a=120.0, **kwargs): # Added **kwargs
        super().__init__()
        self.l_a = l_a
        self.u_a = u_a
        
        # Branch 1: Feature Gating (Squeeze-Excitation)
    ...
    def forward(self, x):
        ...
        combined_quality = 0.0 * mag_quality + 1.0 * sem_quality
        return combined_quality, norm


ATTENTION_REGISTRY = {
    "dual_attention": PowerMagAttention, # This will now use the powerful version
}
...
class AttenDualPartialFC(nn.Module):
    def __init__(self, embedding_size, num_classes, scale=64.0, m1=0.5, m2=0.1,
        ...
        # ... existing init code ...
        self.u_m = 0.6  # Added this
        self.l_m = 0.3 # Added this
        self.current_epoch = 0
        self.attn_warmup_epochs = 5
    ...
    def forward(self, embeddings, labels):
        # 1. Get Quality and Norm from the powerful net
        # Use .detach() for quality net stability as in your original script
        ...
        # 3. Calculate Adaptive Margin (MagFace Principle)
        # Low quality (0) -> low margin (0.3), High quality (1) -> high margin (0.8)
        u_m, l_m = 0.6, 0.3 
        m1_dynamic = (self.u_m - self.l_m) * quality_score + self.l_m

        # 4. Target Margin Logic
        theta = torch.acos(cosine)
        one_hot = torch.zeros_like(cosine)
        one_hot.scatter_(1, labels.view(-1, 1), 1.0)

        target_cosine = torch.cos(theta + m1_dynamic)
        non_target_cosine = torch.cos(theta - self.m2) # Keep your m2 repulsion

        output = one_hot * target_cosine + (1.0 - one_hot) * non_target_cosine
        output = output * self.scale

        # 5. MagFace Regularization (The "Power" Term)
        # Pulls high-quality features to have larger norms
        # lambda_g is usually around 20-35
        lambda_g = 30.0
    ...
    for epoch in range(start_epoch, cfg.num_epoch):
        ...
        for img, labels in train_loader:
            ...
            if global_step % cfg.frequent == 0:
                ...
                lr = opt.param_groups[0]['lr'] # Add this line back
                
                with torch.no_grad(): # Fixed indentation
                    current_norm = torch.norm(embeddings, p=2, dim=1).mean().item()

                logging.info(
                    f"★ MAG-POWER [{cfg.attention_type}] "
