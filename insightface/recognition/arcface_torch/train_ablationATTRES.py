...
BASELINE:  dual_attention (Updated to Residual Logic with Smart Init)
ABLATIONS:
...
class DualAttention(nn.Module):
    """
    IMPROVED BASELINE (Residual Logic): 
    1. Uses Residual/Skip connection logic.
    2. Smart Init: Starts at Score=1.0 (Fixes m1 problem).
    3. BatchNorm + Hardswish for stability.
    """
    def __init__(self, in_features, **kwargs):
        super(DualAttention, self).__init__()
        
        # Increased capacity for better Pose handling (CPLFW)
        hidden_dim = 256 
        
        self.block = nn.Sequential(
            nn.Linear(in_features, hidden_dim, bias=False),
            nn.BatchNorm1d(hidden_dim),   # Stability
            nn.Hardswish(),               # Better activation
            nn.Linear(hidden_dim, 1)      # Output delta
        )
        
        # --- SMART INITIALIZATION (Fixes Attn:0.500) ---
        # Initialize weights to 0. Output of block starts at 0.
        # Sigmoid(0) = 0.5.
        # Final Score = 0.5 + 0.5 = 1.0 (Full Margin Start)
        nn.init.constant_(self.block[3].weight, 0)
        nn.init.constant_(self.block[3].bias, 0)
    ...
    def forward(self, x):
        ...
        # Map to [0, 1] (Starts at 0.5)
        raw_score = self.sigmoid(delta)
        
        # Shift to [0.5, 1.5] (Starts at 1.0)
...
def get_config(attention_type, dataset_path=None):
    ...
    # --- MODIFICATION: DROPOUT ---
    # Used for Backbone to prevent overfitting on CPLFW/CALFW
    config.dropout = 0.4  
    ...
    # --- MODIFICATION: INCREASED NEGATIVE MARGIN ---
    # Increased from 0.1 to 0.15 to reduce False Positives
    config.m2 = 0.15 
...
ABLATION_INFO = {
    "dual_attention": ("★ BASELINE", "Residual Attention with Smart Init"),
...
def train(cfg, rank, local_rank, world_size):
    ...
    # Model
    # --- MODIFICATION: Pass Dropout Config ---
    backbone = get_model(
        cfg.network, 
        dropout=cfg.dropout, # Set to 0.4 from config
        fp16=cfg.fp16, 
    ...
    # --- MODIFICATION: SPLIT OPTIMIZER (10x LR for Attention) ---
    opt = torch.optim.SGD(
        [
            {"params": backbone.parameters(), "lr": cfg.lr},
            # Boost the Attention Head learning rate to wake it up
            {"params": module_fc.parameters(), "lr": cfg.lr * 10} 
        ],
        lr=cfg.lr,
